...
@Component({
  selector: 'app-player',
  imports: [BtnState, BtnMute, BtnBack, BtnTrack, Modal, Result, NgClass, AsyncPipe],
  templateUrl: './player.html',
  styleUrl: './player.css',
  host: {
    '(mousemove)': 'focusControls()'
  }
})
export class Player implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.pathPrefix = this.configSrv.videoSourcesPath$.value;
  ...
  private prepareVideoFile() {
    ...
    this.titleSrv.setTitle('HASD | ' + this.playingNum);
    this.videoFile = `${this.pathPrefix + this.playingNum}.mp4`;
  }
  ...
  focusControls() { this.controls.nativeElement.focus(); }
